/*
*
* Implementation code for the Kojaku-Masuda algorithm (C++ version)
*
*
    ...
    vector<double>& p_values)
{
    ...
    double sig_nq = 0;
    for (int s = 0; s < S; s++) {
        mu_n += nhat[s] / (double)S;
        mu_q += qhat[s] / (double)S;
    }
    for (int s = 0; s < S; s++) {
    ...
    for (int k = 0; k < K; k++) {
        ...
        double denom = 0.0;
        for (int s = 0; s < S; s++) {
            double qbar = qhat[s] + sig_nq * sig_nq / (sig_nn * sig_nn) * (double)(n[k] - nhat[s]);

            double t = sig_nn * (q[k] - qbar) / (sqrt(sig_nn * sig_nn * sig_qq * sig_qq - sig_nq * sig_nq * sig_nq * sig_nq) * h);
            double cum = normcdf(t);
            double w = exp(-(double)pow(n[k] - nhat[s], 2) / (2.0 * h * h * sig_nn * sig_nn)) + 1e-33;
